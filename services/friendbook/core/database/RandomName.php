...
class randomNameGenerator {
	...
	private function getList($type) {
        $filePath = ''.BASE_URL.'assets/datasets/username/';
		$json = file_get_contents($filePath.$type . '.' . $this->inputFormat, FILE_USE_INCLUDE_PATH );
	...
	public function generateNames($num) {
		...
		foreach( $count as $name ) {
                ...
                unset($first_names[$random_fname_index]);
                unset($last_names[$random_lname_index]);
