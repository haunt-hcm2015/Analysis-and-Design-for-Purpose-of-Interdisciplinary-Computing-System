<?php 
 class User{
    ...
    public function indexingData(){
        $smtp = $this->pdo->prepare("SELECT * from `sites` where 1");
        $smtp->execute();
          
        while ($smtp->fetch(PDO::FETCH_ASSOC)){
            $rows[] = $smtp->fetch(PDO::FETCH_ASSOC);
        }
    ...
    public function getUserInfo($userID){
        $smtp = $this->pdo->prepare("SELECT `profile_image`, `username` from `users` WHERE `user_id` = :userID");
